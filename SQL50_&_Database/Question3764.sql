...
WITH high_performer AS(
    ...
    completion_date, 
    AVG(course_rating) OVER (PARTITION BY user_id) AS avg_course_rating,
    COUNT(course_id) OVER (PARTITION BY user_id) AS course_count
    FROM course_completions
),
transition_count AS (
    SELECT user_id, first_course,
    LEAD(first_course) OVER (PARTITION BY user_id ORDER BY completion_date) AS second_course
    FROM high_performer
    WHERE avg_course_rating >= 4 AND course_count >=5
)
