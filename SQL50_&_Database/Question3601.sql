...
WITH `1st_half` AS (
    SELECT D.driver_id, D.driver_name, AVG(distance_km/fuel_consumed) as first_half_avg
...
`2nd_half` AS (
    SELECT D.driver_id, D.driver_name, AVG(distance_km/fuel_consumed) as second_half_avg
    FROM drivers D JOIN trips T
    ON D.driver_id = T.driver_id
...
FROM `1st_half` F JOIN `2nd_half` S
