...
WITH CTE AS (
    SELECT E.employee_id, E.employee_name, E.department, YEARWEEK(M.meeting_date, 1) as week_number, SUM(M.duration_hours) as hours
    FROM Employees E JOIN Meetings M
    ON E.employee_id = M.employee_id
    GROUP BY Employee_id, employee_name, department, YEARWEEK(M.meeting_date, 1)
)
SELECT employee_id, employee_name, department, COUNT(week_number) AS meeting_heavy_weeks
FROM CTE
WHERE hours > 20
GROUP BY employee_id, employee_name, department
HAVING COUNT(week_number) > 1
