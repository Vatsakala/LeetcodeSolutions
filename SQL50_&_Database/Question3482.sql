...
WITH RECURSIVE emp_tree AS (
    SELECT M.employee_id, M.employee_name, 1 AS level, M.manager_id, salary
    FROM Employees M
    WHERE M.manager_id IS NULL

    UNION ALL

    SELECT E.employee_id, E.employee_name, M.level + 1 AS level, E.manager_id, E.salary
    FROM emp_tree M JOIN Employees E
...
sub_tree AS (
    ...
    SELECT M.manager_id, E.employee_id as sub_id
    FROM sub_tree M JOIN Employees E
    ON M.sub_id = E.manager_id
),
team_budget AS (
    SELECT S.manager_id, 
    count(S.sub_id) as team_size,
    SUM(E.salary) as budget
    FROM sub_tree S JOIN emp_tree E
    ON S.sub_id = E.employee_id
...
E.salary + COALESCE(T.budget, 0) as budget
FROM emp_tree E LEFT JOIN team_budget T
